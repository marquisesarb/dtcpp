...
namespace dtcpp {
    ...
    inline double baseCount30360(const DateTime& startDate, const DateTime& endDate) {return 360.0*(endDate.getYear()-startDate.getYear()) + 30.0*(endDate.getMonth()-startDate.getMonth());}

    inline double yfAct360(const DateTime& startDate, const DateTime& endDate) {return double((endDate-startDate).getTotalNanoSeconds())/FACTOR360;}
    inline double yfAct365(const DateTime& startDate, const DateTime& endDate) {return double((endDate-startDate).getTotalNanoSeconds())/FACTOR365;}
    inline double yfAct364(const DateTime& startDate, const DateTime& endDate) {return double((endDate-startDate).getTotalNanoSeconds())/FACTOR364;}
    ...
    inline DatetimeSequence bondSchedule(
        ...
        std::set<DateTime> dates; 
        if (includeStartDate) dates.insert(adjustDate(startDate, holidayCalendar, businessDayConvention));
        if (includeMaturityDate) dates.insert(adjustDate(maturityTenor.getForwardDate(startDate, 1), holidayCalendar, businessDayConvention)); 

        for (int i = 0; i<n-1; i++) {

            DateTime unadjustedDate = frequencyTenor.getForwardDate(startDate, i+1);
            dates.insert(adjustDate(unadjustedDate, holidayCalendar, businessDayConvention));
        }
        return {dates};
    }
}